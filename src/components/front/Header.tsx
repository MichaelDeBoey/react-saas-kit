            ...
            <nav className="relative flex items-center justify-between sm:h-10 md:justify-center" aria-label="Global">
              ...
              <div className="hidden md:absolute md:flex md:items-center md:justify-end md:inset-y-0 md:right-0 z-40">
                <span className="inline-flex rounded-md shadow">
                  <Link
                    to={authenticated ? "/app" : "/login"}
                    className="inline-flex items-center px-4 py-2 border border-transparent text-base font-medium rounded-sm text-theme-600 dark:text-white bg-white dark:bg-slate-800 hover:bg-gray-50"
                  >
                    {!authenticated ? <div>{t("account.shared.signIn")}</div> : <div>{t("shared.enter")}</div>}
                  </Link>
                </span>
              ...
              <div className="rounded-lg bg-slate-50 dark:bg-slate-800 border border-gray-200 dark:border-gray-700 shadow-xl ring-1 ring-black ring-opacity-5 overflow-hidden">
                <div className="px-5 pt-4 flex items-center justify-between">
                  <div>
                    {(() => {
                      if (theme === Theme.DARK) {
                        return <img alt="Icon" className="h-8 w-auto sm:h-10" src={IconDark} />;
                      } else {
                        return <img alt="Icon" className="h-8 w-auto sm:h-10" src={IconLight} />;
                      }
                    })()}
                  </div>
                  <div className="-mr-2">
