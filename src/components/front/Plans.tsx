...
import { setBillingPeriod, setSelected } from "@/store/modules/pricingReducer";
...
export default function Plans() {
  ...
  const products = useSelector((state: RootState): SubscriptionProductDto[] => {
    const items = (state.pricing.products as SubscriptionProductDto[])
  ...
  const customPlan = useSelector((): SubscriptionProductDto | undefined => {
    return products.find((f) => f.contactUs);
  });
  const testProducts = useSelector(() => {
    return !products || products.filter((f) => f.id === undefined || f.id === "").length > 0;
  });
            ...
            if (loading) {
              ...
              return (
                <div className="space-y-6">
                  ...
                  <div className={classNames("mt-16 grid gap-6 lg:gap-3", products.length === 2 && "lg:grid-cols-2", products.length > 2 && "lg:grid-cols-3")}>
                    {products
                            ...
                            >
                              ...
                              <div className="flex-1 space-y-6">
                                ...
                                <ul className="flex-1 space-y-4">
                                  {plan.features.map((feature, idxFeature) => {
                                    return (
                                      <li key={idxFeature}>
