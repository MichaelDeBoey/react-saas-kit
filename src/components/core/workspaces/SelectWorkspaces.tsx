...
interface Props {
  ...
  allowSearch?: boolean;
  onClosed?: () => void;
  onSelected: (workspaces: WorkspaceDto[]) => void;
}

const SelectWorkspaces = ({ maxSize, allowSearch, onClosed, onSelected }: Props, ref: Ref<RefSelectWorkspaces>) => {
  ...
  function isSelected(item: WorkspaceDto) {
  ...
  const filteredItems = (items: WorkspaceDto[]): WorkspaceDto[] => {
    if (!items) {
      return [];
    }
