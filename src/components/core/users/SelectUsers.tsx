...
interface Props {
  allowSearch?: boolean;
  onClosed?: () => void;
  onSelected: (users: UserDto[]) => void;
}

const SelectUsers = ({ allowSearch, onClosed, onSelected }: Props, ref: Ref<RefSelectUsers>) => {
  ...
  function reload() {
    ...
    services.tenantUsers
      .getAll()
      .then((response) => {
        setItems(response.map((f) => f.user));
      })
      .catch((error) => {
  ...
  function isSelected(item: UserDto) {
  ...
  const filteredItems = (items: UserDto[]): UserDto[] => {
    if (!items) {
      return [];
    }
    return items.filter(
