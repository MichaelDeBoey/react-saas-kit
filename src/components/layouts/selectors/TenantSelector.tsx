...
export default function TenantSelector({ className, onAdd }: Props) {
  ...
  return (
    <div ref={clickOutside} className={classNames(className, "relative shadow-2xl")}>
            ...
            {myTenants.map((tenant, index) => {
              return (
                <li key={index} role="option">
                  ...
                  >
                    <div>
                      {tenant.icon && <img alt="Tenant icon" className="h-8 w-8 rounded-sm overflow-hidden float-left mr-2 -ml-1 mt-1" src={tenant.icon} />}
                      ...
                      <p className="text-xs">{getPlanFromTenant(tenant)}</p>
                    </div>
                  </button>
                </li>
              );
