...
import { setCurrentWorkspace } from "@/store/modules/tenantReducer";
...
export default function WorkspaceSelector({ className, onAdd, onSelected }: Props) {
  ...
  const workspaces = useSelector((state: RootState): WorkspaceDto[] => {
    return state.tenant.workspaces ?? [];
  });
  const currentWorkspace = useSelector((state: RootState): WorkspaceDto | null => {
    return state.tenant.currentWorkspace;
  });
  const sortedItems = useSelector((): WorkspaceDto[] => {
    return workspaces
        ...
        <span className="inline-block w-full rounded-sm shadow-sm">
          ...
          >
            <div className="font-bold truncate pr-7 sm:hidden">
              {(() => {
                if (currentWorkspace) {
                  return <span className="truncate">{currentWorkspace.name}</span>;
                } else {
            ...
            <div className="font-bold truncate pr-7 hidden sm:block sm:w-full">
              {(() => {
                if (currentWorkspace) {
                  return <span className="mt-4 truncate">{currentWorkspace.name}</span>;
                } else {
        ...
        >
          <div className="z-40 absolute object-top mt-1 w-full rounded-sm bg-white shadow-lg">
            ...
            <div className="max-h-60 rounded-sm text-sm leading-5 shadow-xs overflow-auto focus:outline-none sm:text-sm sm:leading-5">
              {sortedItems.map((workspace, index) => {
                return (
                  <button
                    key={index}
                    role="option"
