...
export interface RefLinkSelector {
  select: (link: LinkDto) => void;
}

interface Props {
  className?: string;
  onSelected: (id: string, link: LinkDto) => void;
}

const LinkSelector = ({ className = "", onSelected }: Props, ref: Ref<RefLinkSelector>) => {
  ...
  const currentWorkspaceId = () => {
    ...
    return tenant.currentWorkspace?.id ?? "";
  };
  const filteredItems = (): LinkDto[] => {
    if (!items) {
      return [];
    }
    return items.filter(
