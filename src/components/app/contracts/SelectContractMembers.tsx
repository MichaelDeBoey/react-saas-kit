...
export interface RefSelectContractMembers {
  show: (_link: LinkDto, _linkId: string, _selected: string[]) => void;
}
...
const SelectContractMembers = ({ onSelected, onClosed, maxSize = "sm:max-w-lg" }: Props, ref: Ref<RefSelectContractMembers>) => {
  ...
  const [items, setItems] = useState<WorkspaceUserDto[]>([]);
  ...
  function getNoMembers() {
    if (link?.providerWorkspace && link.clientWorkspace) {
      return t("app.tenants.members.noMembersFoundIn", [link?.clientWorkspace.name, link?.providerWorkspace.name]);
    }
    return t("app.tenants.members.noMembers");
  }
  const filteredItems = (): WorkspaceUserDto[] => {
    if (!items) {
      return [];
    }
    return items.filter(
                                        ...
                                        <div className="min-w-0 flex-1 flex items-center space-x-3">
                                          <div className="min-w-0 flex-1">
                                            <p className="text-sm font-bold text-gray-900 truncate">
                                              {item.user.firstName}
                                              {item.user.lastName}
                                              <span className="text-xs font-normal">({item.user.email})</span>
                                            </p>
