...
interface Props {
  maxSize?: string;
  onClosed?: () => void;
  onSelected: (users: EmployeeDto[]) => void;
}

const SelectEmployees = ({ onSelected, onClosed, maxSize = "sm:max-w-lg" }: Props, ref: Ref<RefSelectEmployees>) => {
  ...
  const filteredItems = (): EmployeeDto[] => {
    if (!items) {
      return [];
    }
    return items.filter(
